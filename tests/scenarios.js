...
function testFighterVsScout() {
    ...
    // Give P2 a defensive scout (stationary)
    p2.units.scouts.push({ x: 600, y: 400, targetX: 600, targetY: 400, health: 200, maxHealth: 200, cooldown: 0 });

    const stopCondition = () => p1.units.fighters.length === 0 || p2.units.scouts.length === 0;

    const res = runSimulation(stopCondition);

    recordResult("1 Fighter vs 1 Scout (Head-on)", "Combat", {
        ...
        p2ScoutsLeft: p2.units.scouts.length,
        p2ScoutsHP: p2.units.scouts.reduce((acc, s) => acc + s.health, 0),
    });
}

function testFighterVsTwoScouts() {
    resetGameState();
    const p1 = createDummyPlayer(0, 100, 400);
    const p2 = createDummyPlayer(1, 900, 400);

    // Give P1 a fighter pointing to loop over the scouts
    ...
    // Give P2 two defensive scouts
    p2.units.scouts.push({ x: 600, y: 380, targetX: 600, targetY: 380, health: 200, maxHealth: 200, cooldown: 0 });
    p2.units.scouts.push({ x: 600, y: 420, targetX: 600, targetY: 420, health: 200, maxHealth: 200, cooldown: 0 });

    const stopCondition = () => p1.units.fighters.length === 0 || p2.units.scouts.length === 0;

    const res = runSimulation(stopCondition);

    recordResult("1 Fighter vs 2 Scouts", "Combat", {
        timeSeconds: res.timeSeconds,
        p1FightersLeft: p1.units.fighters.length,
        p1FightersHP: p1.units.fighters.reduce((acc, f) => acc + f.health, 0),
        p2ScoutsLeft: p2.units.scouts.length,
...
function testTwoFightersVsScout() {
    resetGameState();
    const p1 = createDummyPlayer(0, 100, 400);
    const p2 = createDummyPlayer(1, 900, 400);
    ...
    recordResult("2 Fighters vs 1 Scout", "Combat", {
        timeSeconds: res.timeSeconds,
        p1FightersLeft: p1.units.fighters.length,
        p1FightersHP: p1.units.fighters.reduce((acc, f) => acc + f.health, 0),
        p2ScoutsLeft: p2.units.scouts.length,
...
function testEconomyMiningSpeed() {
    ...
    p1.isCPU = true;
    // Start with 1 scout (to capture) and 1 miner
    p1.units.scouts.push({ x: 100, y: 400, targetX: 100, targetY: 400, health: 200, maxHealth: 200, cooldown: 0 });
...
function testUncontestedMapTakeover() {
    ...
    p1.isCPU = true;
    // Just start with a single scout to begin the expansion
    ...
    function checkTakeover() {
        const p1Hull = getConvexHull([p1.homePlanet, ...p1.units.scouts]);
        return asteroids.every(a => {
            let x = a.x, y = a.y, inside = false, vs = p1Hull;
    ...
    recordResult("Time to takeover gameboard with no combat", "Economy", {
        timeSeconds: res.timeSeconds,
        p1Energy: p1.energy,
        p1ScoutsBuilt: p1.units.scouts.length
    });
}
...
function testThreeFightersVsCombo() {
    ...
    // Give P2 one fighter and two scouts
    ...
    p2.units.scouts.push({ x: 600, y: 370, targetX: 600, targetY: 370, health: 200, maxHealth: 200, cooldown: 0 });
    p2.units.scouts.push({ x: 600, y: 430, targetX: 600, targetY: 430, health: 200, maxHealth: 200, cooldown: 0 });

    const stopCondition = () => p1.units.fighters.length === 0 || (p2.units.fighters.length === 0 && p2.units.scouts.length === 0);

    const res = runSimulation(stopCondition);

    recordResult("3 Fighters vs 1 Fighter + 2 Scouts", "Combat", {
        timeSeconds: res.timeSeconds,
        p1FightersLeft: p1.units.fighters.length,
        p1FightersHP: p1.units.fighters.reduce((acc, f) => acc + f.health, 0),
        p2DefendersLeft: p2.units.fighters.length + p2.units.scouts.length,
        p2DefendersHP: p2.units.fighters.reduce((acc, f) => acc + f.health, 0) + p2.units.scouts.reduce((acc, s) => acc + s.health, 0),
    });
}
...
function testCPUvsCPU() {
    ...
    // Standard starting scouts
    p1.units.scouts.push({ x: 150, y: 400, targetX: 150, targetY: 400, health: 200, maxHealth: 200, cooldown: 0 });
    p2.units.scouts.push({ x: 850, y: 400, targetX: 850, targetY: 400, health: 200, maxHealth: 200, cooldown: 0 });
    ...
    const stopCondition = () => p1.homePlanet.health <= 0 || p2.homePlanet.health <= 0 ||
        (p1.units.scouts.length + p1.units.fighters.length + p1.units.miners.length === 0 && p1.energy < 25) ||
        (p2.units.scouts.length + p2.units.fighters.length + p2.units.miners.length === 0 && p2.energy < 25);
    ...
    recordResult("Standard game (CPU vs CPU)", "Full Game", {
        ...
        p1TotalUnitsLeft: p1.units.scouts.length + p1.units.fighters.length + p1.units.miners.length,
        p2TotalUnitsLeft: p2.units.scouts.length + p2.units.fighters.length + p2.units.miners.length,
    });
}
...
const metadata = {
    units: {
        scout: { health: 200, damage: 5, firerate_cd: 0.3 },
